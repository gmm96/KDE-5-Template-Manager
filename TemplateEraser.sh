...
function get_select_templates_msg() {
    ...
    case "${lang_arr[0]:0:2}" in
        "es")
            select="Selecciona plantilla(s):"
            ;;
        "fr")
            select="Sélectionnez le(s) modèle(s):"
            ;;
        "it")
            select="Seleziona modello/i:"
            ;;
        "de")
            select="Vorlage (n) auswählen:"
            ;;
        "pt")
            select="Selecione o(s) modelo(s):"
            ;;
        "ru")
            select="Выберите шаблон(ы):"
            ;;
        *)
            select="Select template(s):"
            ;;
    esac
    echo "$select"
}
